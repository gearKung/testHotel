...
export default {
  ...
  methods: {
    async fetchDashboardSummary() {
      ...
      if (!headers) return;
      try {
        const response = await axios.get('/api/owner/hotels/dashboard/sales-summary', { headers });
    ...
    async fetchDailyChartData() {
      ...
      try {
        const response = await axios.post('/api/owner/hotels/dashboard/daily-sales', requestBody, { headers });
    ...
    async fetchMonthlyChartData() {
      ...
      try {
        const response = await axios.post('/api/owner/hotels/dashboard/monthly-sales', requestBody, { headers });
    ...
    setPeriod(period) {
      ...
      // flatpickr 인스턴스의 clear 메서드를 사용하여 선택된 날짜를 초기화
    ...
    async fetchDashboardActivity() {
      ...
      if (!headers) return;
      try {
        const response = await axios.get('/api/owner/hotels/dashboard/activity', { headers });
    ...
    async fetchHotels() {
      ...
      // 2. API 호출 직전
      console.log("2. fetchHotels: /api/owner/hotels/my-hotels API 호출 시작");
    ...
    async fetchRooms(hotelId) {
      const headers = this.getAuthHeaders();
      if (!headers) return;
      console.log("1. [객실 조회] API 호출 시작:", `/api/owner/hotels/${hotelId}/rooms`);
      try {
    ...
    async updateHotel(formData) {
      ...
      console.log("1. [수정] API 호출 시작:", `/api/owner/hotels/${this.editingHotel.id}`);
    ...
    async deleteHotel(id) {
      ...
      console.log("1. [삭제] API 호출 시작:", `/api/owner/hotels/${id}`);
    ...
    async cancelReservation(reservationId) {
      ...
      try {
        ...
        await axios.post(`/api/owner/hotels/reservations/${reservationId}/owner-cancel`, {}, { headers });
    ...
    async fetchReviews() {
        ...
        if (!headers) return;
        try {
            const response = await axios.get('/api/owner/hotels/dashboard/reviews', { headers });
    ...
    async handleReplySubmit() {
        ...
        try {
            if (this.selectedReview.replied) {
                await axios.put(`/api/owner/hotels/reviews/replies/${this.selectedReview.replyId}`, replyDto, { headers });
                alert("답변이 수정되었습니다.");
            } else {
                await axios.post(`/api/owner/hotels/reviews/${this.selectedReview.id}/reply`, replyDto, { headers });
                alert("답변이 등록되었습니다.");
            }
    ...
    async fetchReservations() {
        ...
        try {
            const response = await axios.get(`/api/owner/hotels/owner/${this.user.id}/reservations`, { headers });
     ...
     async handleReportReview() {
      ...
      try {
        // 새로 만든 신고 API 호출
        await axios.post(`/api/owner/hotels/reviews/${reviewId}/report`, {}, { headers });
